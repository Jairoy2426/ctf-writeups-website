/**
 * ============================================================================
 * Footer Component
 * ============================================================================
 * 
 * @author Hercules (Jairoy2426)
 * @license MIT
 * @copyright 2026 Hercules
 * 
 * Made by Hercules
 * ============================================================================
 */

import Link from 'next/link'
import Image from 'next/image'

export default function Footer() {
  const currentYear = new Date().getFullYear()

  return (
    <footer className="border-t border-dark-border bg-dark-bg mt-auto">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {/* Disclaimer Section */}
        <div className="mb-8 p-6 bg-dark-surface rounded-lg border border-dark-border">
          <h3 className="text-sm font-semibold text-dark-text-muted uppercase tracking-wider mb-3">
            Disclaimer
          </h3>
          <p className="text-dark-text-muted text-sm leading-relaxed">
            All writeups are based on retired or permitted labs. No active machines or paid-only 
            content is disclosed. This site is for educational purposes only.
          </p>
        </div>

        {/* Footer Links */}
        <div className="flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-3">
            <Image
              src="/logo.png"
              alt="Hercules Logo"
              width={32}
              height={32}
              className="rounded"
            />
            <span className="text-dark-text-muted">Writeups</span>
          </div>

          <nav className="flex items-center gap-6 text-sm">
            <Link 
              href="/" 
              className="text-dark-text-muted hover:text-dark-text transition-colors"
            >
              Home
            </Link>
            <a 
              href="https://github.com/Jairoy2426/ctf-writeups" 
              target="_blank" 
              rel="noopener noreferrer"
              className="text-dark-text-muted hover:text-dark-text transition-colors flex items-center gap-2"
            >
              <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                <path fillRule="evenodd" clipRule="evenodd" d="M12 2C6.477 2 2 6.477 2 12c0 4.42 2.865 8.17 6.839 9.49.5.092.682-.217.682-.482 0-.237-.008-.866-.013-1.7-2.782.604-3.369-1.34-3.369-1.34-.454-1.156-1.11-1.463-1.11-1.463-.908-.62.069-.608.069-.608 1.003.07 1.531 1.03 1.531 1.03.892 1.529 2.341 1.087 2.91.831.092-.646.35-1.086.636-1.336-2.22-.253-4.555-1.11-4.555-4.943 0-1.091.39-1.984 1.029-2.683-.103-.253-.446-1.27.098-2.647 0 0 .84-.269 2.75 1.025A9.578 9.578 0 0112 6.836c.85.004 1.705.114 2.504.336 1.909-1.294 2.747-1.025 2.747-1.025.546 1.377.203 2.394.1 2.647.64.699 1.028 1.592 1.028 2.683 0 3.842-2.339 4.687-4.566 4.935.359.309.678.919.678 1.852 0 1.336-.012 2.415-.012 2.743 0 .267.18.578.688.48C19.138 20.167 22 16.418 22 12c0-5.523-4.477-10-10-10z" />
              </svg>
              GitHub
            </a>
          </nav>

          <p className="text-dark-text-muted text-sm">
            © {currentYear} All rights reserved
          </p>
        </div>

        {/* Watermark */}
        <div className="mt-8 pt-6 border-t border-dark-border text-center">
          <div className="flex items-center justify-center gap-2">
            <Image
              src="/logo.png"
              alt="Hercules Logo"
              width={20}
              height={20}
              className="rounded opacity-70"
            />
            <p className="text-dark-text-muted text-xs font-mono tracking-widest">
              Made by <span className="text-white font-semibold">HERCULES</span>
            </p>
          </div>
        </div>
      </div>
    </footer>
  )
}
